      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            {teamleden.length} teamleden geregistreerd
          </p>
        </div>
        <button
          onClick={() => {
          ...
          className={`
            ...
            ${showForm
              ...
              : 'bg-[var(--rb-primary)] text-white hover:bg-[var(--rb-primary-dark)] shadow-[var(--rb-primary)]/20 hover:shadow-[var(--rb-primary)]/30'
            }
          `}
        >
          {showForm ? (
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-6 p-4 bg-gray-50 rounded-xl border border-gray-100">
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="text-sm font-medium text-gray-700 group-hover:text-gray-900">Admin rechten</span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer group">
                <div className="relative">
                  <input
                ...
                <span className="text-sm font-medium text-gray-700 group-hover:text-gray-900">Actief</span>
              </label>
            </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-gray-100 border-dashed">
          <div className="w-20 h-20 mx-auto mb-4 bg-gray-100 rounded-2xl flex items-center justify-center">
            <TeamIcon className="w-10 h-10 text-gray-400" />
          </div>
          <p className="text-gray-500 font-medium text-lg">Geen teamleden gevonden</p>
          <p className="text-gray-400 text-sm mt-1">Voeg je eerste teamlid toe om te beginnen</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {teamleden.map((teamlid, index) => (
            <div
              key={teamlid.id}
              className="
                bg-white rounded-2xl shadow-[var(--shadow-card)] border border-gray-100
            ...
            >
              <div className="p-6">
                ...
                <div className="mt-4 flex items-center gap-2 text-sm text-gray-500">
                  ...
                  <span className="truncate">{teamlid.email}</span>
                </div>

                {/* Bio */}
                {teamlid.bio && (
                  <p className="text-sm text-gray-500 mt-3 line-clamp-2">{teamlid.bio}</p>
                )}
                ...
                <div className="flex items-center gap-2 mt-4 flex-wrap">
                  ...
                  {!teamlid.actief && (
                    <span className="px-2.5 py-1 bg-red-50 text-red-700 rounded-lg text-xs font-semibold border border-red-100">
                      Inactief
                    </span>
                  )}
                  ...
                  {teamlid.discipline && (
                    <span 
                      ...
                      style={{ 
                        backgroundColor: `${disciplineColors[teamlid.discipline as Discipline]}15`,
                        color: disciplineColors[teamlid.discipline as Discipline],
                        borderColor: `${disciplineColors[teamlid.discipline as Discipline]}30`
                      }}
                    >
                      {disciplineLabels[teamlid.discipline as Discipline]}
                    </span>
                  )}
                </div>
                ...
                <div className="flex gap-2 mt-5 pt-5 border-t border-gray-100">
                  <button
                    ...
                    className="flex-1 px-4 py-2.5 text-sm font-semibold text-[var(--rb-primary)] bg-[var(--rb-light)] rounded-xl hover:bg-[var(--rb-primary)] hover:text-white transition-all duration-200"
                  >
                    Bewerken
                  </button>
                  <button
                    onClick={() => handleResetPassword(teamlid.id, `${teamlid.voornaam} ${teamlid.achternaam}`)}
                    className="px-4 py-2.5 text-sm font-semibold text-amber-600 bg-amber-50 rounded-xl hover:bg-amber-100 transition-all duration-200"
