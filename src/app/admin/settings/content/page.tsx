...
export default function VisualContentManagementPage() {
  ...
  async function handleTogglePublished(sectionName: string) {
    const section = filteredContent.find(c => c.section === sectionName);
    if (!section) return;

    const result = await togglePublished(section.id);
    if (result.success) {
      await loadContent();
    }
  }
        ...
        <div className="flex items-center justify-between px-4 h-16">
          ...
          <div className="flex items-center gap-4">
            <Link href="/admin" className="flex items-center gap-2 text-white font-bold">
              ...
              <span className="hidden sm:block">RevaBrain</span>
            </Link>
